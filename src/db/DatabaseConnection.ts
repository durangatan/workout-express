...
export default class DatabaseConnection {
  ...
  query(sql: string, args?: any): Promise<Array<any>> {
    return new Promise((resolve, reject) => {
      ...
      this.connection.query(sql, this.connection.escape(args), (err, rows) => {
        if (err) return reject(err);
        console.log(`DatabaseConnection: executed query: ${sql}`);
        const ensureArray = rows instanceof Array ? rows : Array(rows);
        resolve(ensureArray);
      });
    });
  }
  ...
  performTransactionally(queries: Array<Promise<any>>) {
    return new Promise((resolve, reject) => {

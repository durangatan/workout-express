...
import { ExerciseService, RoutineService, RoutineSetService } from '../logic';
...
let sets: Array<WorkoutSetArguments> = [
  ...
  { exercise: new Exercise({ name: 'Pulldown' }), weight: 90, repetitions: 12 },
  {
...
export default async function seed() {
  ...
  const routine = await RoutineService.insert(joeRoutine);
  const newExercises = await ExerciseService.insertMulti(exercises);
  const newSets = sets.map(set => {
    set.exerciseId = newExercises.find(
      exercise => exercise.name === set.exercise.name
    ).id;
    return set;
  });
  const allSets = await WorkoutSetService.insertMulti(newSets);
  let ordering = 0;
  await allSets.forEach(async set => {
    if (set.type === 'Warmup') {
      await RoutineSetService.insert({
      ...
      for (var i = 0; i < 3; i++) {
        RoutineSetService.insert(
          new RoutineSet({
            routineId: routine.id,
            setId: set.id,
            ordering
          }).insertInto('routine_sets')
        );
        ordering++;
      }
    }

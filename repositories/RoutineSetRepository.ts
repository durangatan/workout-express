...
class RoutineSetRepository {
  ...
  byId(workoutSetId) {
    return this.db.query(
      `SELECT * FROM routine_sets WHERE id = ${workoutSetId};`
    );
  }
  byRoutineId(routineId) {
    return this.db.query(
      `SELECT * FROM routine_sets WHERE routineId =${routineId}`
    );
  }
  ...
  upsertMulti(routineSets) {
    return this.deleteMulti(routineSets).then(() => {
      return Promise.all(
        routineSets.map(routineSet => this.insert(routineSet))
      );
    });
  }
  ...
  insert(routineSet) {
    return this.db
      .query(routineSet.insertInto('routine_sets'))
